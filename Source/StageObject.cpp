...
// 入力: 初期座標, モデルパス, スケール, テクスチャパス, 当たり判定半径, 設置時のY軸オフセット
// 出力: なし / 副作用: 描画用Modelインスタンスの動的生成と、必要に応じた専用テクスチャの上書き適用
...
// 入力: なし / 出力: なし
// 副作用: 動的確保したModelインスタンスを確実に破棄し、オブジェクト消滅時のメモリリークを防ぐ
...
// 入力: なし / 出力: なし
// 副作用: オブジェクト生成直後の1フレーム目のみ地形の高さに合わせて接地座標を計算し、以降はその座標でモデルを更新する
...
// 入力: なし / 出力: なし
// 副作用: 内部で保持しているModelインスタンスへ描画処理を委譲する（描画バッファへの登録）
