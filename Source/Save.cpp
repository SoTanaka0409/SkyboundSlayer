...
/// @details 選択カーソルの初期化、およびセーブ画面用UIテクスチャの動的生成
Save::Save()
	:Select(0)
	, SelectMax(0)
	, SelectMin(0)
...
/// @details Lキーでのセーブ画面呼び出し、およびBACKキーでの画面閉じる（キャンセル）処理のトグル制御
void Save::Update()
{
...
/// @param date = 読み込むセーブデータのポインタ
/// @details 指定されたスロットのデータをゲーム内ステータスへ展開
void Save::GameLoad(SaveDate* date)
{
}

/// @param date = 保存するセーブデータのポインタ
/// @details セーブデータリストへの新規追加、または既存スロット番号との合致時はデータの上書き保存処理
...
/// @details 上下キー入力に応じた、メニュー内の選択カーソル（Select）のインクリメント
