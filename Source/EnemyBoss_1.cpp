...
void EnemyBoss_1::DeathEnemy()
{
	...
	{
		isDead = false;
		mpInventory->DropInventory(mnChance, mbWeapon, mbItem);//キャラクラスのストラクトで初期化する
		player->mpHaveMoney->AddMoney(mfHaveMoney);
		player->mpLevelUp->AddXp(mfHaveXp);
		mfHp = mfMaxHp;//回復させて再スポーン
		mvPosition = VGet(mvPosition.x, 100.0f, mvPosition.z);//反対側にスポーンさせる
