...
void Enemy::Move()
{
	...
	//////////////遘ｻ蜍墓婿蜷代→繧ｹ繝斐・繝峨・蜃ｦ逅・///////////////
	{
		...
		{
			WalkTimer = rand()%300;//縺薙・繧ｿ繧､繝槭・繧帝℃縺弱ｋ縺ｨ譁ｹ蜷題ｻ｢謠・
			mnAlgorithm = rand()%4;
			WalkCount = 0;
			if (mnAlgorithm == 0)//蟾ｦ譁ｹ蜷代∈縺ｮ遘ｻ騾・
			{
				moveVec = VAdd(moveVec, leftMoveVector);
			}
			if (mnAlgorithm == 1)//蜿ｳ譁ｹ蜷代∈縺ｮ遘ｻ騾・
			{
				//veVec.x = 1.0f;
				moveVec = VAdd(moveVec, VScale(leftMoveVector, -1));

			}
			if (mnAlgorithm == 2)//螂･譁ｹ蜷代∈縺ｮ遘ｻ騾・
			{
				moveVec = VAdd(moveVec, UpMoveVector);

			}
			if (mnAlgorithm == 3)//謇句燕譁ｹ蜷代∈縺ｮ遘ｻ騾・
			{
...
void Enemy::DeathEnemy()
{
	...
	if (mpModel->IsAnimationLoopFinish())
	{
		...
		mfHp = mfMaxHp;//蝗槫ｾｩ縺輔○縺ｦ蜀阪せ繝昴・繝ｳ
		mvPosition = VGet(-mvPosition.x, 100.0f, -mvPosition.z);//蜿榊ｯｾ蛛ｴ縺ｫ繧ｹ繝昴・繝ｳ縺輔○繧・
		if (Master::TutorialFlag) {mvPosition=VGet(10000,0,100000); Master::TutorialCount++; }//繝√Η繝ｼ縺ｨ繝ｪ繧｢繝ｫ縺ｯdelete
