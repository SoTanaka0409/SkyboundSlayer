    ...
    : mpEnemyManager(enemyManager), mDifficulty(diff), mCurrentPhase(Phase::PHASE_1), mShopTimer(0), mFadeAlpha(0), mBossPortalPos(VGet(0,0,0))
{
    ...
    new Stage(VAdd(mBossPortalPos, VGet(0.0f, -570.0f, 0.0f)), "Resource/3D/ì]ëóêw/source/portal.mv1", "Resource/3D/ì]ëóêw/source/portal.mv1", VGet(portalSize, portalSize, portalSize));
}
...
void GameManager::Update()
{
    ...
    if (mCurrentPhase == Phase::FADE_OUT_TO_BOSS) {
        ...
        if (mFadeAlpha >= 255) {
            mFadeAlpha = 255;
            mCurrentPhase = Phase::BOSS; // ‰∏ÊôÇÁöÑ„Å´BOSS„Å´„Åó„Å¶Âá∫Áèæ„Åï„Åõ„Ç
            SpawnPhaseEnemies(); // „Åì„Åì„Åß„Éú„ÇπÂá∫Áèæ

            mCurrentPhase = Phase::FADE_IN_BOSS;
            
            // „Éú„Çπ„Ç®„É™„Ç¢„Å∏„Éó„É¨„Ç§„É§„Éº„Çí„ÉØ„Éº„Éó„Åï„Åõ„Çã
        ...
        return; // „Éï„Çß„Éº„Éâ‰∏≠„ÅØ‰ªñ„ÆÊõ¥Êñ∞„Çí„Çπ„Ç≠„É„É
...
void GameManager::SpawnPhaseEnemies()
{
    ...
    auto p = Master::mpPlayer;
    Player3D* player = p->CastTo<Player3D>();
    // Ë¨®ΩµÁ∏∫Âæå„ÅõÁπù„ª„ÉªÁπßΩ∏Á∏∫‰πùÔΩâËûüÊÇ∂ΩåÁ∏∫Ω¶ÈóúÔΩΩËç≥‰πù„ªËùìÂå∫Ω≤Ω°Á∏∫Âä±‚ÜëÁ∏∫„ÉªΩàÁ∏∫„Éª‚ÜìÁ∏≤ºåonfigÁ∏∫ΩÆÁπßΩπÁπù„ª„ÉªÁπßΩ∏Ëç≥Ω≠Ë†¢„ÉªΩ∫ΩßËÆìÂê∂ΩíË≤âÔΩßÁ∏∫Â†∫ΩΩÂí≤ΩΩΩÆÁ∏∫ΩÆËùìÔΩ∫Ë≤ÊÇ∂‚ÜíÁ∏∫Âê∂Ω
    ...
    if (mCurrentPhase == Phase::PHASE_1) {
        ...
        // Wave 3: È©•Âüº„çºÈÇèÂ£ªΩªΩ£ÈÄïÔΩ®
