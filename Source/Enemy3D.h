...
/// @details 3Dモデルの描画に加え、ワールド座標に追従する空間UI（HPバーなど）の描画命令をバッファに登録する
    void Draw() override;

/// @details 3D地形との接地判定（レイキャスト）や重力落下を加味して、親クラスのAIステートと座標を更新する
    void Update() override;

/// @details 3D専用の攻撃モーションを再生し、Z軸を含んだ立体的な攻撃判定（Sphere等）を空間に生成する
