...
void Camera::Initialize()
{
	mpTarget = nullptr;
	//カメラのクリッピング距離の設定
	SetCameraNearFar(100.0f, Config::CameraFar);//100～50000までの距離が見える
...
void Camera::Update()
{
   ...
   // 画面揺れ処理
		...
		if (mpTarget != nullptr)
		{
			...
			//mvLookAtPosition = VAdd(pPlayer->GetPosition(), temp);

			//mvPosition = VAdd(mvLookAtPosition,temp);
		...
		//回転設定
		//MV1SetRotationXYZ(moveVec, mvRotation);
	}
	if (Camera3 == true)
	{
		Camera1 = false;
