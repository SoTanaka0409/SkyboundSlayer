...
{
public:
/// @param filename(モデルパス), pos(初期座標), value(貨幣価値)
/// @details モデルリソースのロードと、跳ね上がり演出用の初期パラメータ設定
    Coin(std::string filename, VECTOR pos, int value);

/// @details 描画用モデルインスタンスの破棄とメモリ解放
    ~Coin();

/// @details プレイヤー吸い寄せや物理挙動の状態更新
    void Update();

/// @details モデル描画命令のバッファ登録
    void Draw();

private:
/// @details 生成直後の座標微調整による「跳ねる」挙動（演出上のカタルシス）の更新
    void UpdatePopPhysics();

/// @details プレイヤーとの距離を判定し、範囲内であれば自動で吸い寄せる移動処理
