...
/// @param damage(被ダメージ量)
/// @details 無敵時間(is_invisible_)をチェックした上でHPを減算し、被弾アニメーションやヒットストップのトリガーを発火させる
...
/// @details 索敵コライダー内で最も距離が近い敵オブジェクトを抽出し、ロックオン対象（target_）として設定する
	void SearchEnemy();
...
/// @details 指定方向へ高速移動しつつ、一時的に無敵フラグ(is_invisible_)を有効化して敵の攻撃をすり抜ける処理を行う
	void Evasion();
...
/// @details 移動先のベクトルに対してモデルのY軸回転角度(angle_)を徐々に補間し、不自然な瞬間振り向きを防止する
	void RotationByMove();
