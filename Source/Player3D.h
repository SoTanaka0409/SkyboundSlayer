...
public:
	...
	void Evasion();
	void CollDelete();
	void UpdateTargetLock();//targetがいる状態か.これがないとtargetがいなくなった瞬間にnullになる
...
private:
	...
	const float kRotateSpeed = 0.2f;//回転速度
	const float kJumpPower = 30.0f;//ジャンプ力

	bool is_jumping_;//ジャンプモーションをするか否か
	bool is_jump_falling_;//落下する座標にいったら下降をするフラグ
	bool has_reached_jump_peak_;//目標座標に行ったときにtrue
	bool is_jump_collider_active_;
	bool is_attack_slide_target_found_;

	float jump_power_;//ジャンプ力
	float speed_;
	...
	int upgrade_evasion_invincibility_ = 0; // 追加無敵フレーム
	int invincible_timer_ = 0; // 現在の無敵タイマー
	// -----------------------------------

	int attack_slide_count_;//攻撃間隔をカウントする
	int attack_jump_count_;
	int attack_count_;

	int attack_slide_cooldown_ = 200;//攻撃可能時のクールタイム
	int attack_cooldown_ = 30;
	int attack_jump_cooldown_=200;
	////////////////	攻撃力	

	float attack_;//ダメージを計算するときに使われる基本的な値
	float normal_attack_;//何らかの要因でmfAttackの値を変えた時に戻すための値
	float jump_attack_;
	float slide_attack_;
	float bullet_attack_;
	VECTOR move_vec_ = VGet(0.0f, 0.0f, 0.0f);//移動方向
	VECTOR previous_move_vec_;
	VECTOR attack_slide_direction_;//移動方向
	VECTOR attack_slide_step_;//目標とする座標

	//ステージとの当たり判定をする
	bool is_invisible_;

	int target_search_count_;//一番近い敵
	...
	bool is_stage_out_;//trueだったらoldPositionに戻るようにする
};
