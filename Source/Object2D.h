...
public:
    // 入力: initPos (初期のスクリーン座標) / 出力: なし
    // 副作用: 2D座標、描画フラグ、生存フラグなどの内部状態を安全な初期値にセットアップする
    ...
    // 入力: なし / 出力: なし
    // 副作用: 派生クラスのUIアニメーションや位置計算を毎フレーム進行させるための仮想更新メソッド
    virtual void Update();

    // 入力: なし / 出力: なし
    // 副作用: 現在の座標・回転状態を適用し、2Dスプライトやテキストを描画バッファへ登録するための仮想描画メソッド
    virtual void Draw();
